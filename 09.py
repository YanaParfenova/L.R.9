...
class TicTacToe:
    ...
    def init_main_window(self):
        self.window = tk.Tk()
        self.window.title("Выберите сторону - Крестики или Нолики")
        ...
        tk.Button(self.window, text="Нолики", command=lambda: self.start_game("O")).pack(pady=5)
    ...
    def player_move(self, i, j):
        if self.buttons[i][j]['text'] == "":
            ...
            if self.check_winner():
                ...
                messagebox.showinfo("Игра окончена", "Это ничья!")
                self.end_game()
            else:
                self.bot_move()
